...
Class TemplateController extends Controller{
	...
	public function content(Request $request){
		...
		$invoice = ($request->has('invoice_id')) ? \App\Invoice::find($request->input('invoice_id')) : null;
		$quotation = ($request->has('quotation_id')) ? \App\Quotation::find($request->input('quotation_id')) : null;

		if(!$template || (!$user && !$invoice && !$quotation))
	        return response()->json(['status' => 'error']);

		$mail_data = $this->templateContent(['slug' => $template->slug,'user' => $user,'invoice' => $invoice,'quotation' => $quotation]);
